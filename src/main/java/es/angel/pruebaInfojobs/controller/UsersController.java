...
public class UsersController extends HttpController {
    ...
    public Response doGet(final Map<String, String> parameters) {
        ...
        if (userName != null && userName.trim().length() > 0) {
            userData = new Users.Builder().withUserName(userName).build().findByUserName()
                    .toString().split(":");
        } else {
            final List<Users> users = new Users().readAll();
            userData = users.stream().map(Users::toString).collect(Collectors.joining(":")).split(":");
        }
        final String body = viewResolver.resolveView(accept, userData);
    ...
    public Response doDelete(final Map<String, String> parameters) {
        ...
        return new Response.Builder().withStatusCode(204).build();
    }
